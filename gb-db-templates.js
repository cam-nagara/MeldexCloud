...
const _CHAR_WEIGHT_BMI_FORMULA = _dbTplFormulaSwitch('prop("体型")', [
  ['美容体重', 19.5], ['ぽっちゃり(細)', 18], ['痩せ型', 22], ['ぽっちゃり', 25],
...
function _resolveDbTemplatePropertyTypeForPath(typeConfig, dbPath) {
  ...
  if (targetSheet) {
    ...
    const targetPath = _dbTemplateNormalizePath(_dbTemplateSiblingDbPath(dbPath, targetSheet));
    out.relationDb = targetPath === currentPath ? '' : targetPath;
...
function _saveDbTemplatePropertyTypesToBackend(dbPath, propertyTypes) {
  if (typeof apiPut !== 'function') return Promise.resolve(false);
  ...
  return apiPut('/db-metadata?path=' + encodeURIComponent(dbPath), { property_types: nextTypes })
    .then(() => {
      if (typeof state !== 'undefined' && state.currentDbPath === dbPath) {
        ...
        state.dbMetadata.property_types = _cloneTemplateData(nextTypes);
      }
      return true;
    })
    .catch(e => {
      console.warn('シートテンプレートのプロパティ型保存に失敗:', e);
      return false;
    });
}
...
function applyDbTemplate(dbPath, template, opts) {
  ...
  template.properties.forEach(p => {
    if (!overwrite && c.propertyTypes[p.name]) {
      ...
      c.propertyTypes[p.name] = _resolveDbTemplatePropertyTypeForPath(p.type, dbPath);
      applied.push(p.name);
    }
  });
