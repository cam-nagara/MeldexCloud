...
function showEntityColMenu(e) {
  ...
  const showFooter = getDbViewConfig(dbPath).showFooter || false;
  ...
  if (dbPath) {
    ...
    _addMenuRenameInput(menu, dbName, async (newName) => {
      try {
        await apiPut('/rename', { path: dbPath, new_name: newName });
        showStatus('シート名を変更しました');
        if (typeof refreshOutliner === 'function') refreshOutliner();
        // 新しいパスを推定して再表示
        const newPath = dbPath.replace(/[^/]+$/, newName);
        if (typeof selectDatabase === 'function') selectDatabase(newPath);
      } catch (err) { showStatus('シート名変更失敗: ' + (err.message || err), true); }
    }, { placeholder: 'シート名を変更...' });
  }

  const pinnedColsList = getPinnedCols(dbPath);
  const entityPinned = getDbViewConfig(dbPath).entityColumnPinned !== false; // デフォルト true
  const items = [
    ...
    { type: 'submenu', label: lucide('settings2', 14) + ' 表示設定',
      children: [
        { type: 'submenu', label: '集計行', children: [
          { label: (showFooter ? radioMark(true) : '　') + '表示', action: () => { const c = getDbViewConfig(dbPath); c.showFooter = true; saveDbViewConfig(dbPath, c, { historyLabel: 'シート表示: 集計行', historyDetail: '表示' }); renderPivot(); }},
          { label: (!showFooter ? radioMark(true) : '　') + '非表示', action: () => { const c = getDbViewConfig(dbPath); c.showFooter = false; saveDbViewConfig(dbPath, c, { historyLabel: 'シート表示: 集計行', historyDetail: '非表示' }); renderPivot(); }},
        ]},
...
function _showLegacyAdvancedFilterModal() {
  ...
  document.body.appendChild(o);
  const list = o.querySelector('#adv-filter-list');
  if (list) {
...
function createAdvancedFilterRowElement(f, idx, allProps) {
  ...
  ['*', ...filterable].forEach((prop) => {
    const opt = document.createElement('option');
    opt.value = prop;
    opt.textContent = prop === '*' ? '全プロパティ' : prop;
    if (f?.property === prop) opt.selected = true;
...
function clearAdvFilters() {
  setAdvancedFilters(state.currentDbPath, []);
  document.querySelector('.modal-overlay').remove();
  renderPivot();
}
...
function getPropertyTypes(dbPath) {
  ...
  if (state.dbMetadata && 'property_types' in state.dbMetadata && state.dbMetadata.property_types !== null) {
    return state.dbMetadata.property_types;
  }
...
async function _deleteColumn(dbPath, propName) {
  ...
  const oldPT = oldCfg.propertyTypes?.[propName];

  const c = getDbViewConfig(dbPath);
  // colOrder から除去
  if (c.colOrder) c.colOrder = c.colOrder.filter(n => n !== propName);
  // colWidths / propertyTypes / columnLocks から除去
  if (c.colWidths) delete c.colWidths[propName];
