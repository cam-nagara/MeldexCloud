(function () {
  ...
  const PANEL_ITEMS = [
    ...
    { label: 'ビューワー', icon: 'monitor', action: () => _callGlobal('openRightPanelTab', 'preview') },
    { label: 'オプション', icon: 'panelRight', action: () => _callGlobal('toggleOptionPanel') },
    { label: 'バージョン', icon: 'gitBranch', action: () => _callGlobal('openRightPanelTab', 'version') },
  ...
  const ANNOTATION_ITEMS = [
    ...
    { id: 'close', label: '閉じる', icon: 'x', action: () => _callGlobal('closeAnnotationToolbar') },
  ];
  ...
  function _hasBlockingModal() {
    return Array.from(document.querySelectorAll(MODAL_SELECTOR)).some((el) => {
  ...
  function _formatBlock(tag) {
    _restoreSelection();
    if (typeof rtHeading === 'function' && tag !== 'P') rtHeading(tag);
  ...
  function _createLink() {
    ...
    if (!url) return;
    _execTextCommand('createLink', url);
  }
  ...
  function _ensureAnnotationBar() {
    ...
    ANNOTATION_ITEMS.forEach((item) => {
      ...
      if (item.action === _toggleAnnotationVisibility) button.dataset.annMobileVisibility = '1';
      row.appendChild(button);
    });
