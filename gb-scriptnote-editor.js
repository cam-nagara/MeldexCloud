/* gb-scriptnote-editor.js: flattened split script for static cloud hosting. */
/* Source chunk: gb-scriptnote-editor.part01.js */
...
function _sn2BuildRubyMarkup(base, ruby) {
  if (!base) return '';
  if (!ruby) return _sn2EscapeRubyText(base);
...
function _sn2SplitRawTextByVisibleOffset(rawText, visibleOffset) {
  ...
  segments.forEach((segment) => {
    ...
    const plain = (segment.type === 'ruby' || segment.type === 'manual-link')
      ? segment.plain
      : _sn2UnescapeRubyText(segment.raw);
...
function _sn2ReplaceRawTextByVisibleRange(rawText, startOffset, endOffset, replacement) {
  ...
  return before + _sn2EscapeRubyText(String(replacement || '')) + after;
}
...
class ScriptNoteEditor {
  ...
  loadDoc(parsed, path = '') {
    ...
    if (this._path && typeof _sn2Editors !== 'undefined') _sn2Editors[this._path] = this;
    this._render();
    this._pushUndo('初期状態');
  }
  ...
  _resolveCharaColors(chara, colId) {
    ...
    if (chara.autoColor) {
      ...
      const target = (typeof act === 'object' && colId) ? (act[colId] || 'bg') : (typeof act === 'string' ? act : 'bg');
      if (target === 'both') {
  ...
  _applyRowStyle(rowEl, role) {
    ...
    const applyGutterStyle = (el, colStyleKey) => {
      ...
      el.style.background = r.bgColor || pageBg;
    ...
    rowEl.querySelectorAll('.sn2-custom-cell').forEach(cell => {
      ...
      const ecCustom = this._resolveCharaColors(chara);
      const roleCustom = { bgColor: ecCustom.bgColor, textColor: ecCustom.textColor };
  ...
  _render() {
    ...
    for (let i = 0; i < this.doc.rows.length; i++) {
      ...
      const prevRow = i > 0 ? this.doc.rows[i - 1] : null;
      const prevCalc = i > 0 ? calc[i - 1] : null;
      allRowEls.push(this._buildRowEl(this.doc.rows[i], i, calc[i], mergeDisplay, prevRow, prevCalc, customCols));
...
/* Source chunk: gb-scriptnote-editor.part02.js */
...
/* Source chunk: gb-scriptnote-editor.part03.js */
          targetIdx += dir;
        }
...
/* Source chunk: gb-scriptnote-editor.part04.js */
      targetOffset = 0;
    } else {
      let last = null;
      let node;
