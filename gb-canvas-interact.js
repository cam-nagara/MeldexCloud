...
function bdInitInteraction(root) {
  ...
  let erasing = false, erasedNodeIds = null, erasedConnRefs = null;
  ...
  function updateTouchPinch(e) {
    ...
    bd.zoom = newZoom;
    const angleDelta = (touchAngle(a, b) - touchPinch.startAngle) * (180 / Math.PI);
  ...
  function onCanvasPointerdown(e) {
    ...
    if (nodeEl && !bd.editing) {
      ...
      // ドラッグ開始（選択中の全ノード）
      drag = nid; canvas.classList.add('bd-node-dragging'); bdPushUndo();
      dragOffsets = {};
      ...
      Object.keys({...dragOffsets}).forEach(id => {
        ...
        if (pn && pn._followChildren) {
          bdDescendants(id).forEach(did => {
            if (!dragOffsets[did]) { const dn=bd.nodes.find(v=>v.id===did); if(dn && !dn.locked) dragOffsets[did]=dragOffsetEntry(dn); }
          });
        }
      });
  ...
  function onDocPointermove(e) {
    ...
    if (_cspRotate) {
      ...
      const delta = (curAngle - _cspRotStartAngle) * (180 / Math.PI);
    ...
    if (bd._rightDragNode) {
      ...
      if (rd.dragged) {
        ...
        if (preview) {
          ...
          if (n) {
            const _w = typeof bdScreenToWorld === 'function' ? bdScreenToWorld(e.clientX, e.clientY) : { x: (e.clientX - canvas.getBoundingClientRect().left - bd.panX) / bd.zoom, y: (e.clientY - canvas.getBoundingClientRect().top - bd.panY) / bd.zoom };
            preview.setAttribute('x1', n.x + (rd.nodeEl.offsetWidth||100)/2);
            preview.setAttribute('y1', n.y + (rd.nodeEl.offsetHeight||60)/2);
            preview.setAttribute('x2', _w.x);
            preview.setAttribute('y2', _w.y);
          }
        }
    ...
    if (drag && dragOffsets && !bd._dragExtracted) {
      const firstO = dragOffsets[Object.keys(dragOffsets)[0]];
      if (firstO) {
        const dx0 = Math.abs(e.clientX - firstO.sx) / bd.zoom;
        const dy0 = Math.abs(e.clientY - firstO.sy) / bd.zoom;
        if (dx0 >= 8 || dy0 >= 8) {
    ...
    if (drag && dragOffsets) {
      if (!dragMoved && bdDragMovedBeyondClickThreshold(dragOffsets, e)) dragMoved = true;
      let rawDx = _bdPointerDelta(e.clientX - (dragOffsets[Object.keys(dragOffsets)[0]]?.sx || 0)) / bd.zoom;
      let rawDy = _bdPointerDelta(e.clientY - (dragOffsets[Object.keys(dragOffsets)[0]]?.sy || 0)) / bd.zoom;
      // Shift軸固定: 大きい方の軸のみ移動
      if (e.shiftKey) { if (Math.abs(rawDx) > Math.abs(rawDy)) rawDy = 0; else rawDx = 0; }
      for (const [id, o] of Object.entries(dragOffsets)) {
        const n=bd.nodes.find(v=>v.id===id);
        const dx = e.shiftKey ? rawDx : _bdPointerDelta(e.clientX - o.sx) / bd.zoom;
        const dy = e.shiftKey ? rawDy : _bdPointerDelta(e.clientY - o.sy) / bd.zoom;
        if(n) {
    ...
    if (resizing && resizeSelection) {
      const nextW = Math.max(40, resizeOW + _bdPointerDelta(e.clientX-resizeSX)/bd.zoom);
      const nextH = Math.max(28, resizeOH + _bdPointerDelta(e.clientY-resizeSY)/bd.zoom);
      ...
      (bd.connections || []).forEach(c => {
        ...
        try {
          ...
          for (let i = 0; i <= samples; i++) {
            ...
            if (pt.x >= l && pt.x <= l + w && pt.y >= t && pt.y <= t + h) { rectConnIds.add(c.id); break; }
          }
        } catch (_) {}
      });
      let nextConnIds;
  ...
  function onDocPointerup(e) {
    ...
    if (bd._lineToolDrag) {
      ...
      if (targetEl) {
        ...
        if (start.nid) {
          if (bdCanCreateConnection(start.nid, toId)) {
            bdPushUndo();
    ...
    if (drag) {
      ...
      drag=null; dragOffsets=null; dragMoved=false; bdDirty();
    }
    if (resizing) {
  ...
  function onCanvasWheel(e) {
    ...
    bd.zoom = newZoom;
    bd.panX = mx - (mx - bd.panX) * (bd.zoom / oz);
    bd.panY = my - (my - bd.panY) * (bd.zoom / oz);
    bdTransform();
  }
    ...
    for (const f of files) {
      ...
      jobs.push((async () => {
        try {
          ...
          if (isImage && res.path) {
            const imgUrl = API_BASE + '/file-raw?path=' + encodeURIComponent(res.path);
            const linkType = typeof _bdInferLinkType === 'function' ? _bdInferLinkType(res.path, 'image') : 'image';
            return typeof bdCreateNodeWithStyle === 'function'
              ? bdCreateNodeWithStyle('', wx + offsetX, wy, { img: imgUrl, link: res.path, linkType, w: 250 })
              : bdNode('', wx + offsetX, wy, 250, 0, { img: imgUrl, link: res.path, linkType });
          }
          const linkType = typeof _bdInferLinkType === 'function' ? _bdInferLinkType(res.path || '', '') : '';
          return typeof bdCreateNodeWithStyle === 'function'
            ? bdCreateNodeWithStyle(f.name, wx + offsetX, wy, { link: res.path || '', linkType, w: 200 })
            : bdNode(f.name, wx + offsetX, wy, 200, 0, { link: res.path || '', linkType });
        } catch (err) {
