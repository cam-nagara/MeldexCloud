...
function _csvUniqueHeader(base) {
  const used = new Set(_csvHeaders.map(h => String(h)));
  let name = base;
  let i = 2;
  while (used.has(name)) {
    name = `${base} ${i}`;
...
function startCsvCellEdit(td, ri, ci) {
  ...
  if (isMultiline) {
    ...
    td.appendChild(ta);
    ta.focus();
    ta.addEventListener('blur', () => { if (!cancelled) commitCsvCellEdit(td, ta.value, ri, ci); });
    ...
    input.focus();
    input.select();
    input.addEventListener('blur', () => { if (!cancelled) commitCsvCellEdit(td, input.value, ri, ci); });
...
function startCsvHeaderEdit(th, ci) {
  ...
  input.addEventListener('blur', () => {
    if (cancelled) return;
    if (String(_csvHeaders[ci] ?? '') === String(input.value)) {
      _csvSetDecoratedText(th, input.value);
      return;
    }
    const before = _csvSnapshot();
    _csvHeaders[ci] = input.value;
    _csvData[0][ci] = input.value;
    _csvSetDecoratedText(th, input.value);
...
async function saveCsv() {
  ...
  try {
    const content = '\uFEFF' + serializeCsv(_csvData);
    await apiPut('/file?path=' + encodeURIComponent(_csvPath), { content, skip_if_missing: true });
    _csvDirty = false;
...
async function convertCsvToDb() {
  ...
  try {
    await apiPost('/import-csv', { csv_path: _csvPath, db_path: dbPath });
    showStatus('DB変換完了: ' + fileName);
