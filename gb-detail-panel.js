/* gb-detail-panel.js: flattened split script for static cloud hosting. */
/* Source chunk: gb-detail-panel.part01.js */
...
/* Source chunk: gb-detail-panel.part02.js */
...
function fileThemeReset(ctx) {
  ...
  if (ctx === 'board' && typeof bd !== 'undefined') {
    ...
    _saveFileTheme(id ? { __themeId: id } : null);
  }
  if (ctx !== 'scriptnote') {
    const panelId = _fsThemePanelId(ctx);
    if (panelId && typeof clearFileStyleForPanel === 'function') clearFileStyleForPanel(panelId);
...
function _syncDetailPanel(label, path, type) {
  ...
  if (!inPane) {
    ...
    if (rpDetail && typeof _openDetailRightPanel === 'function') _openDetailRightPanel();
  }
  // タブシェルを確保（初回起動時の保険）
  if (rpDetail) _ensureDetailTabShell(rpDetail);
  if (type !== 'board') {
  ...
  if (type === 'page') {
    ...
    _showDatabaseInfoInDetailPanel(label, path);
  }
}
...
/* Source chunk: gb-detail-panel.part03.js */
