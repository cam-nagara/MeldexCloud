...
function _collectTimelineAutoFitEntries(data, cfg) {
...
function autoFitTimelineColumns(ctx, dbPath, options = {}) {
  ...
  const entries = _collectTimelineAutoFitEntries(data, cfg);
  const timeGroups = new Set();
  const rowGroups = new Set();
  entries.forEach(entry => {
    timeGroups.add(roundTimeValue(entry.timeVal, cfg.scale));
    if (entry.endVal) timeGroups.add(roundTimeValue(entry.endVal, cfg.scale));
  ...
  if (!targetKey || targetKey === '__rowHeader') {
    next.colWidths.__rowHeader = _timelineAutoWidthPx(rows, isHorizontal ? (cfg.rowProp === '_entity' ? 'エントリ' : cfg.rowProp) : cfg.timeProp);
  }
  const colTexts = new Map(cols.map(col => [_timelineColKey(col), []]));
  entries.forEach(entry => {
    const key = _timelineColKey(isHorizontal ? roundTimeValue(entry.timeVal, cfg.scale) : entry.rowVal);
  ...
  cols.forEach(col => {
    ...
    next.colWidths[key] = _timelineAutoWidthPx(colTexts.get(key) || [], col);
  });
