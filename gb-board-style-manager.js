/* gb-board-style-manager.js: flattened split script for static cloud hosting. */
/* Source chunk: gb-board-style-manager.part01.js */
...
function _bdLineStyleSnapshotFromConnection(conn, fallback) {
  ...
  return {
    ...
    labelTextColor: eff.labelTextColor || fallback?.labelTextColor || '',
    labelBgColor: eff.labelBgColor || fallback?.labelBgColor || '',
    labelBorderColor: eff.labelBorderColor || fallback?.labelBorderColor || '',
    ...
    textShadowColor: eff.textShadowColor || fallback?.textShadowColor || '',
  };
}
...
/* Source chunk: gb-board-style-manager.part02.js */
...
function _bdOpenStyleManagerPopup(kind, anchorEl, options) {
  ...
  const render = () => {
    ...
    const topBelow = (rect.bottom / zoom) + 4;
    const topAbove = (rect.top / zoom) - (box.height / zoom) - 4;
    // ピッカーは上部に配置されているので、まず下に出す。下に収まらず上にスペースがあれば上に。
    const viewportH = window.innerHeight / zoom;
    popup.style.top = (topBelow + (box.height / zoom) <= viewportH || topAbove < 8
      ? topBelow
      : topAbove) + 'px';
    if (popup.getBoundingClientRect().right > window.innerWidth) {
      popup.style.left = Math.max(4, (window.innerWidth - popup.offsetWidth - 4) / zoom) + 'px';
    }
...
function _bdRenderStyleManagerInPanel(kind, container, selectedId, mode) {
  ...
  _bdBuildStyleFields(fieldsEl, kind, selected, (field) => {
    ...
    _bdRenderStyleManagerInPanel(kind, container, selected.id, 'diff');
    bdRefreshBoardToolbar();
    if (typeof bdRefreshSelectionDetails === 'function') bdRefreshSelectionDetails(true);
  }, {
...
function _bdRenderDepthStyleInPanel(container, selectedIndex, mode) {
  ...
  if (depthFieldsEl && selected && window.gbFmt) {
    const depthEmit = (field) => {
      applyDepthStyles({ fontOnly: field === 'fontFamily' });
      _bdRenderDepthStyleInPanel(container, idx, 'diff');
    };
    ...
    _bdBuildStyleFields(cardFieldsWrap, 'card', selected, depthEmit, {
      hideName: true,
    ...
    _bdBuildStyleFields(lineFieldsWrap, 'line', selected.line, depthEmit, {
      hideName: true,
  ...
  const render = () => {
    ...
    popup.querySelectorAll('[data-bd-depth-item]').forEach(item => {
      ...
      item.addEventListener('drop', event => {
        ...
        opts.currentIndex = targetIndex;
        if (typeof opts.onSelect === 'function') opts.onSelect(targetIndex);
    ...
    popup.querySelector('[data-bd-popup-depth-delete]')?.addEventListener('click', () => {
      if ((bd.depthStyles || []).length <= 1) return;
      if (typeof bdPushUndo === 'function') bdPushUndo();
      bd.depthStyles.splice(opts.currentIndex, 1);
      opts.currentIndex = Math.max(0, Math.min(opts.currentIndex, bd.depthStyles.length - 1));
      if (typeof opts.onSelect === 'function') opts.onSelect(opts.currentIndex);
      applyDepthStyles(render);
    });
