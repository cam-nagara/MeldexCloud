  ...
  const runSyncAction = async (action) => {
    ...
    finally { if (modalApi.isOpen()) setSyncBusy(false); }
  };
  const closeSyncModal = (reason) => {
    setSyncBusy(false);
  ...
  o.querySelector('#sync-ical-export').addEventListener('click', async () => {
    ...
    await runSyncAction(()=>MeldexExportSave.saveUrl('/api/calendar-db/ical/export?path=' + encodeURIComponent(dbPath), {
      filename: stem + '.ics',
      extension: '.ics',
  ...
  o.querySelector('#sync-migrate').addEventListener('click', async () => {
    await runSyncAction(async()=>{try { const res = await apiPost('/calendar-db/migrate-from-sqlite', { db_path: dbPath }); showStatus(`マイグレーション完了: ${res.migrated}件`); closeSyncModal('migrated'); await _refreshCalendarDb(dbPath); } catch (err) { showStatus(syncErrorMessage('マイグレーションに失敗', err), true); }});
  });
}
