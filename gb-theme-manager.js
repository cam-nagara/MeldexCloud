/* gb-theme-manager.js: flattened split script for static cloud hosting. */
/* Source chunk: gb-theme-manager.part01.js */
...
(function (global) {
  ...
  const APP_STYLE_UI_TARGETS = Object.freeze([
    ...
    { id: 'style-calendar', group: 'style', app: 'カレンダー', label: 'カレンダー', props: STYLE_TARGET_PROPS, states: STYLE_TARGET_STATES, propLabels: STYLE_TARGET_PROP_LABELS, stateLabels: { selected: '今日/選択' }, vars: {
  ...
  const LIGHT_VARS = {
    ...
    '--page-h1-bg': 'transparent', '--page-h2-bg': 'transparent', '--page-h3-bg': 'transparent', '--page-h4-bg': 'transparent', '--page-h5-bg': 'transparent', '--page-h6-bg': 'transparent',
    '--page-text-fg': '#1e1e1e', '--page-text-bg': '#ffffff', '--page-link-fg': 'var(--accent2)',
  ...
  const PASTEL_VARS = {
    ...
    '--page-h1-bg': 'transparent', '--page-h2-bg': 'transparent', '--page-h3-bg': 'transparent', '--page-h4-bg': 'transparent', '--page-h5-bg': 'transparent', '--page-h6-bg': 'transparent',
    '--page-text-fg': '#2f3440', '--page-text-bg': '#ffffff', '--page-link-fg': 'var(--accent2)',
  ...
  const EARTH_VARS = {
    ...
    '--page-h1-bg': 'transparent', '--page-h2-bg': 'transparent', '--page-h3-bg': 'transparent', '--page-h4-bg': 'transparent', '--page-h5-bg': 'transparent', '--page-h6-bg': 'transparent',
    '--page-text-fg': '#d9ddd8', '--page-text-bg': '#101210', '--page-link-fg': 'var(--accent2)',
  ...
  const GIRLY_VARS = {
    ...
    '--page-h1-bg': 'transparent', '--page-h2-bg': 'transparent', '--page-h3-bg': 'transparent', '--page-h4-bg': 'transparent', '--page-h5-bg': 'transparent', '--page-h6-bg': 'transparent',
    '--page-text-fg': '#3a1828', '--page-text-bg': '#ffe8f0', '--page-link-fg': 'var(--accent2)',
  ...
  function getOsAccentThemeColorSet() {
    return hasOsAccentRuntimeColor() ? [getOsAccentColor()] : [];
  }

  function _effectiveThemeColorSet(colors, fallback) {
    if (getUseOsAccentColor() && hasOsAccentRuntimeColor()) return getOsAccentThemeColorSet();
  ...
  function applyOsAccentColorSetting(enabled = getUseOsAccentColor(), options = {}) {
    ...
    if (enabled && !hasOsAccentRuntimeColor()) return;
    root.style.setProperty('--theme-os-accent', getOsAccentColor());
  ...
  function _isObsoleteInitialCustomTheme(theme) {
  ...
  function _ensureInitialCustomThemeCleanup() {
    ...
    try {
      ...
      const promotedDefault = _promotedInitialThemeIdFromCustomId(storedDefault)
        || _promotedInitialThemeIdFromName(storedDefault)
        || (_initialBuiltinTargetFromId(storedDefault)?.id || '')
        || _promotedInitialThemeIdFromCustomId(storedEditorTheme)
        || _promotedInitialThemeIdFromName(storedEditorTheme)
        || (_initialBuiltinTargetFromId(storedEditorTheme)?.id || '');
      ...
      const nextThemes = rawThemes.filter(theme => !_isObsoleteInitialCustomTheme(theme));
  ...
  function getCustomThemes() {
    _ensureInitialCustomThemeCleanup();
    return _readRawCustomThemes().filter(t => !_isObsoleteInitialCustomTheme(t));
  }

  function saveCustomThemes(themes, options = {}) {
    const before = _captureThemeSettingsStorage([CUSTOM_THEMES_KEY]);
    _ensureInitialCustomThemeCleanup();
    const nextThemes = (themes || []).filter(t => t && t.id && t.name && !_isObsoleteInitialCustomTheme(t));
    localStorage.setItem(CUSTOM_THEMES_KEY, JSON.stringify(nextThemes));
...
/* Source chunk: gb-theme-manager.part02.js */
/* gb-theme-manager.part02.js: split from gb-theme-manager.js */
  function _themeUiAutoMixCss(toneColor, amount) {
    const percent = _normalizeThemeUiTonePercent(amount, 30);
    return `color-mix(in srgb, var(--theme-slot-color,var(--accent)) ${100 - percent}%, ${toneColor} ${percent}%)`;
  }

  function _themeUiColorCss(value, autoTone) {
    const normalized = _normalizeThemeUiValue(value);
    if (normalized === THEME_UI_VALUE_NONE) return '';
    if (normalized === THEME_UI_VALUE_AUTO) return 'var(--theme-slot-color,var(--accent))';
    if (normalized === THEME_UI_VALUE_AUTO_LIGHT) return _themeUiAutoMixCss('white', autoTone?.light);
    if (normalized === THEME_UI_VALUE_AUTO_DARK) return _themeUiAutoMixCss('black', autoTone?.dark);
    ...
    THEME_UI_TARGETS.forEach(target => {
      _themeUiStatesForTarget(target).forEach(state => {
        ...
        _themeUiPropsForTarget(target, state.id).forEach(prop => {
          const colorCss = _themeUiColorCss(config[target.id]?.[state.id]?.[prop.id], autoTone);
...
`;
  ...
  function withDerivedUiStyleVars(vars) {
    ...
    COMMON_INTEGRATED_APP_STYLE_KEYS.forEach(key => { delete next[key]; });
  ...
  function applyBoardThemeCssVars(board, canvas, world, themeDef) {
    ...
    prevKeys.forEach(key => {
      if (key === '--bd-theme-font-family') {
        if (!hasFileVar('--bd-default-font-family')) {
  ...
  function applyBoardThemeRuntime(board, canvasEl, worldEl) {
    ...
    if (board && typeof global.getCssVar === 'function') {
      ...
      if (!fs || fs['--bd-auto-align'] === undefined) {
        const t = (global.getCssVar('--bd-auto-align') || '').trim();
  ...
  function setBoardTheme(board, id) {
    ...
    board.themeId = _normalizeThemeIdWithPromotedCustom(id) || '';
  ...
  function collectCurrentCssVars() {
    ...
    const cssVars = {};
    keys.forEach(k => {
      if (COMMON_INTEGRATED_APP_STYLE_KEY_SET.has(k)) return;
  ...
  function createCustomThemeFromTheme(sourceId, name) {
    ...
    const next = normalizeCustomThemePayload(source, name || `${source.name} のコピー`);
    next.id = newCustomThemeId('custom');
    next.name = String(name || `${source.name} のコピー`).trim();
    const list = getCustomThemes();
    list.push(next);
    saveCustomThemes(list);
    return clone(next);
  ...
  function importCustomTheme(themeDef, fallbackName) {
    ...
    const list = getCustomThemes().filter(t => t.id !== imported.id);
    if (list.some(t => t.name === imported.name)) imported.name += ' コピー';
    list.push(imported);
    saveCustomThemes(list);
  ...
  function migrateStoredThemeColorSet() {
    ...
    if (_initialBuiltinTargetFromId(currentId)) {
