...
(function () {
  ...
  function _style() {
    ...
    style.textContent = `
      .sa-pwa-install-button{position:fixed;z-index:9989;inset-inline-end:max(12px,env(safe-area-inset-right));inset-block-end:max(12px,env(safe-area-inset-bottom));min-width:44px;min-height:44px;padding:8px 14px;border:1px solid #5b616b;border-radius:999px;background:#2e333b;color:#f3f4f6;box-shadow:0 4px 18px rgb(0 0 0 / 38%);font:600 13px/1.2 system-ui,-apple-system,"Segoe UI",sans-serif;touch-action:manipulation}
      .sa-pwa-install-button:hover{background:#3a414c}.sa-pwa-install-button:focus-visible,.sa-pwa-install-dialog button:focus-visible{outline:3px solid #8bc2f4;outline-offset:2px}
      ...
      .sa-pwa-install-dialog{position:fixed;z-index:10016;inset-inline-start:50%;inset-block-start:50%;translate:-50% -50%;display:grid;gap:14px;width:min(440px,calc(100vw - 28px));max-height:min(620px,calc(var(--sa-visual-height,100dvh) - 28px));box-sizing:border-box;padding:20px;overflow:auto;border:1px solid #4b505a;border-radius:14px;background:#25272d;color:#f3f4f6;box-shadow:0 18px 54px rgb(0 0 0 / 50%);color-scheme:dark;font:14px/1.5 system-ui,-apple-system,"Segoe UI",sans-serif}
      .sa-pwa-install-dialog[hidden],.sa-pwa-install-backdrop[hidden],.sa-pwa-install-button[hidden]{display:none!important}.sa-pwa-install-dialog h2,.sa-pwa-install-dialog p,.sa-pwa-install-dialog ol{margin:0}.sa-pwa-install-dialog p,.sa-pwa-install-dialog ol{color:#c4c8d0}.sa-pwa-install-dialog ol{padding-inline-start:24px}.sa-pwa-install-dialog li+li{margin-block-start:8px}
      .sa-pwa-install-actions{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap}.sa-pwa-install-actions button{min-height:44px;padding:8px 14px;border:1px solid #4b505a;border-radius:8px;background:#30333a;color:#f3f4f6;font:600 14px/1.2 system-ui,-apple-system,"Segoe UI",sans-serif;touch-action:manipulation}.sa-pwa-install-actions .is-primary{border-color:#3f8ed4;background:#286faa}
      .sa-pwa-install-status{min-height:20px;color:#b8bdc8;font-size:12px}.sa-pwa-install-status.is-error{color:#ff9ca2}@media(pointer:coarse){.sa-pwa-install-button,.sa-pwa-install-dialog button{min-height:44px}}`;
