/* gb-canvas-engine.js: flattened split script for static cloud hosting. */
/* Source chunk: gb-canvas-engine.part00.js */
...
/* Source chunk: gb-canvas-engine.part01.js */
...
/* Source chunk: gb-canvas-engine.part02.js */
...
/* Source chunk: gb-canvas-engine.part03.js */
...
function bdDrawConns(options) {
  ...
  if (partialConnIds) {
    partialConnIds.forEach(connId => bdRemoveConnectionRender(svg, defs, connId));
  } else {
    defs.innerHTML = '';
    ...
    document.querySelectorAll('.bd-conn-label').forEach(l => l.remove());
  }
  ...
  bd.connections.forEach(c => {
    ...
    const depthLineStyleIndex = typeof bdGetAutoDepthLineStyleIndexForConnection === 'function'
      ...
      : '';
    if (c.hidden || (bd.displayFilters && bd.displayFilters.showConnections === false)) return; // 非表示
    ...
    if (!selfLoop && !fromA && !toA) {
      ...
      if (!HORIZONTAL_FORCED.has(st) && !VERTICAL_FORCED.has(st)) {
        const auto = _bdAutoRouteByVector(fp, tp, fw, fh, tw, th, GAP);
        if (auto) {
    ...
    hit.addEventListener('dblclick', (ev) => {
      ...
      // textVisible=false のラインはラベル編集不可
      if (connStyle.textVisible === false) return;
      ev.stopPropagation();
...
function bdFinishEdit() {
  ...
  if (el) {
    ...
    if (editedNode) { editedNode.text = txt.innerText.trim(); txt.innerHTML = applyAutoLinks(esc(editedNode.text).replace(/\n/g,'<br>'), bd.path); }
  ...
  if (editedNode && typeof bdMarkNodeDirty === 'function') bdMarkNodeDirty(editedNode.id, 'finish-edit');
  if (editedNode && typeof bdMarkSelectionDirty === 'function') bdMarkSelectionDirty([editedNode.id], 'finish-edit');
  if (editedNode && typeof bdMarkExtrasDirty === 'function') bdMarkExtrasDirty({ minimap: true, boardUi: true, comments: [editedNode.id] }, 'finish-edit');
  ...
  if (editedNode && typeof bd !== 'undefined' && bd.autoAlign !== false) {
...
function bdDeleteSelected() {
  if (!bdGetSelectedConnectionIds().length && bd.selected.size===0) return;
  bdPushUndo();
  if (bd.selected.size === 0 && bdGetSelectedConnectionIds().length) {
    bdGetSelectedConnectionIds().forEach(connId => {
  ...
  const selectedConnIds = new Set(bdGetSelectedConnectionIds());
...
function bdAlign(type) {
  ...
  else if (type==='distributeH') { elems.sort((a,b)=>a.n.x-b.n.x); if(elems.length<3) return; const min=elems[0].n.x, max=elems[elems.length-1].n.x; const step=(max-min)/(elems.length-1); elems.forEach((v,i)=>{v.n.x=min+step*i;}); }
  else if (type==='distributeV') { elems.sort((a,b)=>a.n.y-b.n.y); if(elems.length<3) return; const min=elems[0].n.y, max=elems[elems.length-1].n.y; const step=(max-min)/(elems.length-1); elems.forEach((v,i)=>{v.n.y=min+step*i;}); }
...
function bdFitAll() {
  ...
  bd.zoom = Math.min(cw/w, ch/h, 1.5); bd.zoom = Math.max(0.1, bd.zoom);
...
async function bdSave() {
  ...
  try {
    await apiPut('/file?path='+encodeURIComponent(savePath),{content:markdown, skip_if_missing:true});
...
function bdCloneNodesWithOffset(sourceNodes, offset) {
  ...
  const newNodes = (sourceNodes || []).map(n => {
    const {id: _id, x: _x, y: _y, ...rest} = n;
    const nn = bdNode(n.text, n.x+offset, n.y+offset, n.w, n.h, {...rest, markers: n.markers ? {...n.markers} : undefined});
    idMap[n.id] = nn.id;
    return nn;
  });
  ...
  newNodes.forEach((nn, idx) => {
    ...
    if (source?.contained && !mappedParent) {
      const pos = typeof bdAbsolutePosition === 'function' ? bdAbsolutePosition(source) : null;
...
/* Source chunk: gb-canvas-engine.part04.js */
